# Select mobs to buff
execute as @e[type=#survival_reimagined:hostile,tag=!buffed] run function survival_reimagined:buff_all
...
execute as @a at @s if block ~ ~-1 ~ #minecraft:leaves run advancement grant @s only survival_reimagined/i_cant_stand_you
execute as @a at @s if block ~ ~-1 ~ #minecraft:snow run advancement grant @s only survival_reimagined/hard_to_walk_through