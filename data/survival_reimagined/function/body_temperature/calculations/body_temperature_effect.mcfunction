...
execute at @s if score @s isHeatSourceClose matches 0 store result score @s isHeatSourceClose run clone ~-3 ~-3 ~-3 ~3 ~3 ~3 ~-3 ~-3 ~-3 filtered minecraft:campfire[lit=true] force

function survival_reimagined:body_temperature/calculations/body_temperature_constant

scoreboard players operation @s bodyTemperatureFloat += @s bodyTemperatureConstant
scoreboard players operation @s[scores={bodyTemperatureArmorProtectionLevel=-4..4}] bodyTemperatureFloat += @s bodyTemperatureArmorProtectionLevel
